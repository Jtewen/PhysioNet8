...
def train_12ECG_classifier(input_directory, output_directory):
    ...
    KEEP_PROB = tf.keras.layers.Input(shape=(1,), name="KEEP_PROB")

    e1 = tf.keras.layers.Conv1D(
        filters=48,
        kernel_size=9,
        strides=4,
        padding="valid",
    ...
    e5_0 = tf.keras.layers.MaxPooling1D(
        pool_size=2,
        strides=2
    ...
    for i in range(num_classes):
        e6 = tf.keras.layers.Dropout(rate=1 - KEEP_PROB)(
            tf.keras.layers.Dense(100, activation="relu")(emb)
        )
    ...
    e5_d = e5_p + e5_n
    e6_d = tf.nn.dropout(
        tf.keras.layers.Dense(e5_d, 64, activation=tf.nn.relu), rate=1 - (KEEP_PROB)
    )
    logits_d = tf.keras.layers.Dense(e6_d, len(ref_domains))
