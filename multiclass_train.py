...
def train_12ECG_classifier_multiclass(input_directory, output_directory):
    ...
    AGE_LABELS = tf.compat.v1.placeholder(tf.float32, [None, 1], name="AGE")
    SEX_LABELS = tf.compat.v1.placeholder(tf.float32, [None, 1], name="SEX")
    ...
    # Add layers for age and sex prediction
    logits_age = tf.compat.v1.layers.dense(emb, 1, name="logits_age")  # No activation for regression
    pred_age = logits_age  # Directly use logits if regression, apply activation if needed

    logits_sex = tf.compat.v1.layers.dense(emb, 1, name="logits_sex")  # For binary classification
    pred_sex = tf.nn.sigmoid(logits_sex, name="pred_sex")
    ...
    Loss_d = tf.reduce_mean(
        ...
        # Loss for age prediction (MSE)
    Loss_a = tf.reduce_mean(tf.square(logits_age - AGE_LABELS))

    # Loss for sex prediction (Binary Cross-Entropy)
    Loss_s = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits_sex, labels=SEX_LABELS))
