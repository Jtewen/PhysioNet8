...
def to_one_hot(array, N):
    ...
    labels[np.arange(array.shape[0]), array] = 1.0
    return labels
...
def find_domains(names):
    ...
    print("Number of records: ", len(names))
    domains = []
    ...
    print("Number of domains: ", len(ref_domains))
    ...
    for i in range(len(data)):
        ...
        if np.random.rand() < p_chzero1:
            ...
            if np.random.rand() < 0.5:
                ...
                data[i, :, ind] = lowpass_filter(
                    np.random.normal(0, n_ratio, size=data[i, :, ind].shape), fc
                )
        ...
        if np.random.rand() < p_noise1:
            ...
            inds = np.random.choice(
                np.arange(12), size=np.random.randint(12), replace=False
            ).ravel()
            data[i, :, inds] += np.random.normal(
                0, n_ratio, size=data[i, :, inds].shape
            )
            ...
            t_inds = np.sort(
                np.random.choice(np.arange(length), size=10, replace=False).ravel()
            )

            ai = np.random.randint(6)
            a = t_inds[ai]
            b = t_inds[np.random.randint(ai + 2, 10)]
            data[i, a:b, inds] += np.random.normal(
                0, n_ratio, size=data[i, a:b, inds].shape
            )
        ...
        try:
            if np.random.rand() < p_art1:
                ...
                for ind in inds:
                    ...
                    t_inds = np.sort(
                        np.random.choice(
                            np.arange(length), size=10, replace=False
                        ).ravel()
                    )
                    ai = np.random.randint(6)
                    a = t_inds[ai]
                    b = t_inds[np.random.randint(ai + 3, 10)]
                    data[i, a:b, ind] += lowpass_filter(
                        np.random.normal(0, n_ratio, size=data[i, a:b, ind].shape), fc
                    )
        ...
        if np.random.rand() < p_scale:
            data[i] *= 0.5 + np.random.rand() * 1.0
        elif np.random.rand() < p_scale2:
            inds = np.random.choice(
                np.arange(12), size=np.random.randint(1, 12), replace=False
            ).ravel()
            for ind in inds:
                data[i, :, ind] *= 0.5 + np.random.rand() * 1.0
        ...
        if np.random.rand() < p_inv1:
            ...
            inds = np.random.choice(
                np.arange(12), size=np.random.randint(2, 12)
            ).ravel()
            data[i, :, inds] *= -1.0
